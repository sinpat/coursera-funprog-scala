...
object TimeUsage extends TimeUsageInterface {
  ...
  def row(line: List[String]): Row =
    line match {
      case head :: tl => Row.fromSeq(head +: tl.map(_.toDouble))
      case _ =>
        throw new IllegalArgumentException(
          "Line input for row method does not contain an element!"
        )
    }
  ...
  def classifiedColumns(
    ...
    val (primary, other) = columnNames.partition(name =>
      List("t01", "t03", "t11", "t1801", "t1803").exists(name.startsWith(_))
    )
    val (working, other2) =
      other.partition(name => List("t05", "t1805").exists(name.startsWith(_)))
    val (leisure, _) = other2.partition(name =>
      List(
        "t02",
        "t04",
        "t06",
        ...
        "t16",
        "t18"
      ).exists(name.startsWith(_))
    )

    (primary.map(col(_)), working.map(col(_)), leisure.map(col(_)))
  }
  ...
  def timeUsageSummary(
    ...
    val workCol = df("telfs")
    val workingStatusProjection: Column =
      when(workCol >= 1 and workCol < 3, "working")
    ...
    def sumColsToHours(cols: List[Column]): Column = cols.reduce(_ + _) / 60
    val primaryNeedsProjection: Column =
      sumColsToHours(primaryNeedsColumns).as("primaryNeeds")
    val workProjection: Column = sumColsToHours(workColumns).as("work")
    val otherProjection: Column = sumColsToHours(otherColumns).as("other")
    df
      .select(
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    val workCol = summed("working")
    val sexCol = summed("sex")
    val ageCol = summed("age")
    summed
      .groupBy(workCol, sexCol, ageCol)
      .agg(
  ...
  def timeUsageGroupedTyped(
    ...
    summed
      ...
      .agg(
        round(typed.avg((x: TimeUsageRow) => x.primaryNeeds), 1).as[Double],
        round(typed.avg((x: TimeUsageRow) => x.work), 1).as[Double],
        round(typed.avg((x: TimeUsageRow) => x.other), 1).as[Double]
      )
