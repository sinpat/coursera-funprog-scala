...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  val conf: SparkConf =
    new SparkConf().setMaster("local").setAppName("Wikipedia Ranking")
  ...
  val wikiRdd: RDD[WikipediaArticle] =
    sc.parallelize(WikipediaData.lines.map(WikipediaData.parse(_)))
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    rdd.aggregate(0)(
      (c, article) => if (article.mentionsLanguage(lang)) c + 1 else c,
      _ + _
    )
  ...
  def makeIndex(
      langs: List[String],
      rdd: RDD[WikipediaArticle]
  ): RDD[(String, Iterable[WikipediaArticle])] = rdd
    .flatMap(a => langs.filter(a.mentionsLanguage(_)).map((_, a)))
    .groupByKey
  ...
  def rankLangsReduceByKey(
    ...
    rdd
      .flatMap(a => langs.filter(a.mentionsLanguage(_)).map((_, 1)))
      .reduceByKey(_ + _)
      .collect()
      .toList
      .sortBy(_._2)(Ordering[Int].reverse)
