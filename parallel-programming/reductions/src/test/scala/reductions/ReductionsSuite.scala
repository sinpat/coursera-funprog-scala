...
class ReductionsSuite {
  ...
  @Test def `lineOfSightPar should correctly handle an array of size 4`
      : Unit = {
  ...
  @Test def `countChange should return 0 for money < 0`: Unit = {
    def check(money: Int, coins: List[Int]) =
      assert(
        parCountChange(money, coins, combinedThreshold(money, coins)) == 0,
  ...
  @Test def `countChange should return 1 when money == 0`: Unit = {
    def check(coins: List[Int]) =
      assert(
        parCountChange(0, coins, combinedThreshold(0, coins)) == 1,
  ...
  @Test def `countChange should return 0 for money > 0 and coins = List()`
      : Unit = {
    def check(money: Int) =
      assert(
        parCountChange(money, List(), combinedThreshold(money, Nil)) == 0,
  ...
  @Test def `balance should work for empty string`: Unit = {
    def check(input: String, expected: Boolean) =
      assert(
        parBalance(input.toArray, 2) == expected,
