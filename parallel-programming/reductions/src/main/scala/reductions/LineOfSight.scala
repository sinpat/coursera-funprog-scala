...
object LineOfSight extends LineOfSightInterface {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    for {
      ...
      output(i) = upsweepSequential(input, 1, i + 1)
    }
  }
  ...
  def downsweepSequential(
    ...
    (value, idx) <- input.zipWithIndex.slice(from, until)
  } {
    output(idx) = Math.max(value / Math.max(idx, 1), startingAngle)
  }
