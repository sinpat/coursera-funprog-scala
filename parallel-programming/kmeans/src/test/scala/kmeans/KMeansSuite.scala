...
class KMeansSuite {
  ...
  def checkClassify(
      points: Seq[Point],
      means: Seq[Point],
      expected: Map[Point, Seq[Point]]
  ): Unit =
    assertEquals(expected, classify(points, means))

  @Test def `'classify should work for empty 'points' and empty 'means'`
      : Unit = {
  ...
  @Test def `'classify' should work for empty 'points' and 'means' == Seq(Point(1,1,1))`
      : Unit = {
    val points: Seq[Point] = IndexedSeq()
  ...
  @Test def `kMeans should work duh`: Unit = {
    val res = kMeans(
      Seq(
        new Point(0, 0, 1),
        new Point(0, 0, -1),
      ...
      Seq(new Point(0, -1, 0), new Point(0, 2, 0)),
      0
    )
    assertEquals(Vector((0.0, 0.0, 0.0), (0.0, 5.5, 0.0)), res)
  }
