...
class KMeans extends KMeansInterface {
  ...
  def classify(
    ...
    val grouped = points.groupBy(p =>
      findClosest(p, means)
    ) // some keys (means) may be missing!

    means.foldLeft(grouped)((map, mean) =>
      if (!map.contains(mean)) map.updated(mean, Nil) else map
    )
  }
  ...
  def classify(
    ...
    means.foldLeft(grouped)((map, mean) =>
      if (!map.contains(mean)) map.updated(mean, ParSeq.empty) else map
    )
  }

  def findAverage(oldMean: Point, points: Seq[Point]): Point = if (
    points.isEmpty
  ) oldMean
  else {
    var x = 0.0
  ...
  def findAverage(oldMean: Point, points: ParSeq[Point]): Point = if (
    points.isEmpty
  ) oldMean
  else {
    var x = 0.0
  ...
  def converged(
      ...
      oldMeans: Seq[Point],
      newMeans: Seq[Point]
  ): Boolean = oldMeans.zip(newMeans).forall { case (oldMean, newMean) =>
