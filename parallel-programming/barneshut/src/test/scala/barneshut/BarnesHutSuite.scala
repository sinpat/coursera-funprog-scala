...
class BarnesHutSuite {
  ...
  @Test def `Empty.insert(b) should return a Leaf with only that body (2pts)`
      : Unit = {
    val quad = Empty(51f, 46.3f, 5f)
  ...
  @Test def `Body.updated should take bodies in a Leaf into account (2pts)`
      : Unit = {
    val b1 = new Body(123f, 18f, 26f, 0f, 0f)
  ...
  @Test def `fork with 4 empty`: Unit = {
    val fork = new Fork(
      new Empty(5, 5, 10),
      new Empty(15, 5, 10),
      new Empty(5, 15, 10),
    ...
    val centerY = 10
    assert(fork.size ~= 20)
    assert(fork.centerX ~= centerX)
    assert(fork.centerY ~= centerY)
    assert(fork.mass ~= 0)
    assert(fork.massX ~= centerX, s"${fork.massX} should be $centerX")
    assert(fork.massY ~= centerY)
  }

  @Test def `fork should insert into the correct quadrant`: Unit = {
    val fork = new Fork(
      new Empty(5, 5, 10),
      new Empty(15, 5, 10),
      new Empty(5, 15, 10),
      ...
      new Body(1, 10, 10, 0, 0)
    )
    assert(
      fork.nw.isInstanceOf[Leaf],
      s"nw quadrant, ${fork.nw} should be Leaf"
    )
    assert(fork.nw.total == 1, "nw quadrant should have 1 element")
  }

  @Test def `sector matrix insert out of boundaries should work`: Unit = {
    val boundaries = new Boundaries {
      minX = 0
      minY = 0
      maxX = 10
    ...
    val sectorMatrix = new SectorMatrix(boundaries, 2)
    sectorMatrix += new Body(0, 15, 10, 0, 0)
    sectorMatrix += new Body(0, 10, 15, 0, 0)
    assert(sectorMatrix(1, 1).size == 2)
  }

  @Test def `should combine sectorMatrices correctly`: Unit = {
    ...
    val sm1 = new SectorMatrix(bounds, 2)
    val sm2 = new SectorMatrix(bounds, 2)
    sm1 += new Body(0, 0, 0, 0, 0)
    sm2 += new Body(0, 0, 0, 0, 0)
    sm1 += new Body(0, 10, 10, 0, 0)
    sm2 += new Body(0, 10, 10, 0, 0)
    val combinedSM = sm1 combine sm2
    assert(combinedSM(0, 0).size == 2)
    assert(combinedSM(1, 1).size == 2)
  }

  @Test def `inserting into a leaf that is bigger than threshold should result in fork`
      : Unit = {
    val leaf = new Leaf(10, 10, 10, Seq())
    val leafAfterInsert = leaf.insert(new Body(0, 9, 9, 0, 0))
    assert(
      leafAfterInsert.isInstanceOf[Fork],
      "insert into leaf that is too big did not result in fork"
    )
    val fork = leafAfterInsert.asInstanceOf[Fork]
    assert(
      fork.nw.isInstanceOf[Leaf],
      "nw after insert into leaf is not a Leaf"
    )
  }
