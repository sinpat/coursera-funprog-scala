...
package object barneshut {
  ...
  case class Leaf(
    ...
    def insert(b: Body): Quad = if (size > minimumSize) {
      ...
      val westCenterX = centerX - quadrantSize / 2
      val eastCenterX = centerX + quadrantSize / 2
      val northCenterY = centerY - quadrantSize / 2
      val southCenterY = centerY + quadrantSize / 2
      val emptyFork = Fork(
        Empty(westCenterX, northCenterY, quadrantSize),
        Empty(eastCenterX, northCenterY, quadrantSize),
        Empty(westCenterX, southCenterY, quadrantSize),
        Empty(eastCenterX, southCenterY, quadrantSize)
      )
      bodies.foldLeft(emptyFork)(_ insert _).insert(b)
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int)
    ...
    def +=(b: Body): SectorMatrix = {
      val x =
        ...
        else Math.floor(b.x / sectorSize).toInt
      val y =
        if (b.y >= boundaries.maxY) sectorPrecision - 1
        else Math.floor(b.y / sectorSize).toInt
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      ...
      for {
        i <- 0 until newSM.matrix.size
      } newSM.matrix(i) = matrix(i) combine that.matrix(i)
      newSM
    }
    ...
    def toQuad(parallelism: Int): Quad = {
      ...
      def quad(x: Int, y: Int, span: Int, achievedParallelism: Int): Quad = {
        if (span == 1) {
          ...
          val (nw, ne, sw, se) =
            if (
              parallelism > 1 && achievedParallelism < parallelism * BALANCING_FACTOR
            )
              parallel(
