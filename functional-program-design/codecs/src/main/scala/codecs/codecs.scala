...
trait DecoderInstances {
  ...
  implicit val intDecoder: Decoder[Int] =
    Decoder.fromPartialFunction {
      case Num(value) if (value.isValidInt) => value.toInt
    }

  /** A decoder for `String` values */
  implicit val stringDecoder: Decoder[String] =
    Decoder.fromPartialFunction { case Str(value) =>
      value
    }

  /** A decoder for `Boolean` values */
  implicit val boolDecoder: Decoder[Boolean] =
    Decoder.fromPartialFunction { case Bool(value) =>
      value
    }
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Arr(items) => {
        val decodedOptions = items.map(_.decodeAs[A])
        // turn List[Option] to Option[List]
        decodedOptions.foldLeft(Option(List.empty[A]))((acc, curr) =>
          acc.flatMap(xs => curr.map(xs :+ _))
        )
      }
      case _ => None
    }
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case Obj(fields) => fields.get(name).flatMap(decoder.decode(_))
      case _           => None
    }
...
trait PersonCodecs {
  ...
  implicit val personDecoder: Decoder[Person] =
    Decoder.field[String]("name").zip(Decoder.field[Int]("age")).transform {
...
trait ContactsCodecs {
  ...
  implicit val contactsEncoder: Encoder[Contacts] =
    ObjectEncoder.field("people")(
      Encoder.listEncoder[Person].transform(_.people)
    )

  implicit val contactsDecoder: Decoder[Contacts] =
    Decoder.field("people")(Decoder.listDecoder[Person].transform(Contacts(_)))
}
...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    println("=== My stuff ===")
    println(Encoder.listEncoder[Int].encode(List(1, 2)))

    println("=== Original stuff ===")
