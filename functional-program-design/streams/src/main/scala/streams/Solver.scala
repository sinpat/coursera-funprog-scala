...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(
      b: Block,
      history: List[Move]
  ): LazyList[(Block, List[Move])] = b.legalNeighbors.to(LazyList) map {
    case (b, move) =>
      (b, move :: history)
  }
  ...
  def newNeighborsOnly(
      ...
      explored: Set[Block]
  ): LazyList[(Block, List[Move])] = neighbors filterNot { case (b, _) =>
    explored.contains(b)
  }
  ...
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = from(
    LazyList((startBlock, List.empty)),
    Set(startBlock)
  )
